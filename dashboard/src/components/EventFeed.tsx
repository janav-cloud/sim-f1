...
const EVENT_CONFIG: Record<string, { icon: string; color: string; bg: string }> = {
  ...
  "Virtual Safety Car": { icon: "VSC", color: "text-amber-400", bg: "bg-amber-400/10 border-amber-400/20" },
...
export default function EventFeed({ events, currentLap }: { events: any[]; currentLap: number }) {
  ...
  // Fast stable sort: latest lap first, then latest occurrence within lap first
  ...
  return (
    <div className="flex flex-col gap-2">
      {/* Current lap status if no incidents this lap */}
      {currentLap > 0 && currentLapEvents.length === 0 && (
        <div className="text-center py-2.5 px-3 rounded-lg border border-neutral-800/40 bg-neutral-900/20 text-[10px] uppercase tracking-[0.15em] font-medium text-neutral-600">
          Green Flag Racing &bull; Lap {currentLap}
        </div>
      )}
      ...
      {currentLapEvents.map((event, i) => {
        ...
        return (
          <div key={`current-${event._idx ?? i}`} className={`flex gap-3 items-start p-3 rounded-lg border animate-fade-in ${config.bg}`}>
      ...
      {/* Separator between current lap and previous laps */}
      {currentLapEvents.length > 0 && pastEvents.length > 0 && (
        <div className="flex items-center gap-2 my-2 px-1">
          <div className="flex-1 border-t border-neutral-800/60"></div>
          <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-neutral-600">Previous Laps</span>
          <div className="flex-1 border-t border-neutral-800/60"></div>
        </div>
      )}

      {/* Past events (dimmer with clean card style) */}
      {pastEvents.map((event, i) => {
        const config = EVENT_CONFIG[event.type] || { icon: "ℹ", color: "text-neutral-400", bg: "bg-neutral-800/50 border-neutral-700/50" };
        return (
          <div key={`past-${event._idx ?? i}`} className="flex gap-3 items-start p-2.5 rounded-lg border border-neutral-800/40 bg-neutral-900/30 opacity-60 hover:opacity-100 hover:bg-neutral-900/60 transition-all">
            <div className="flex flex-col items-center min-w-[2.5rem]">
            ...
            <div className={`text-xs mt-1 ${config.color} opacity-70`}>{config.icon}</div>
            <div className="flex-1 min-w-0 mt-0.5">
              <div className={`text-[9px] font-bold uppercase tracking-[0.15em] mb-0.5 ${config.color} opacity-80`}>{event.type}</div>
              <div className="text-xs text-neutral-400 leading-relaxed">{event.message}</div>
            </div>
          </div>
        );
      })}
